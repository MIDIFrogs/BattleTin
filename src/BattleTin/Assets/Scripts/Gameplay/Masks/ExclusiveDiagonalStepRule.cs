...
{
    public sealed class ExclusiveDiagonalStepRule : IMovementRule
    {
        private readonly ExclusiveDirectStepRule direct;
        private readonly DiagonalStepRule diagonal;

        private readonly int everyNTurns;

        public ExclusiveDiagonalStepRule(int maxDistance, int everyNTurns = 1)
        {
            direct = new(maxDistance);
            diagonal = new(maxDistance);
        ...
        public IEnumerable<CellId> GetAvailableCells(PieceState piece, GameState state)
        {
            ...
            return diagonalCells.Except(directCells);
        }
    }
}
