...
{
    public sealed class DiagonalStepRule : IMovementRule
    {
        private readonly int maxSteps;
        private readonly int everyNTurns;

        public DiagonalStepRule(int maxSteps, int everyNTurns = 1)
        {
        ...
        public IEnumerable<CellId> GetAvailableCells(PieceState piece, GameState state)
        {
            ...
            while (frontier.Count > 0)
            {
                ...
                foreach (var next in state.Board.GetDiagonalNeighbors(cell))
                {
                    if (!visited.Add(next))
                    {
